...
typedef struct{
	char NOME[200], DURACAO[20], EP[50], TEMP[50], DESC[10000], AVAL[4], TIPO[15];
	int FS;//Var caso seja Filme = 1, Série = 0
}LIBRARY;
...
void library_add(){
	...
	do{
		...
		if(i==1){//filme
			system("CLS");
			strcpy(produto[quant].TIPO,"Filme");
			scanf("%c", &descarte);
			...
			system("CLS");
			strcpy(produto[quant].TIPO,"Serie");
			scanf("%c", &descarte);
...
void library_Vis(){
	...
	do{
		...
		for(i=1;i<=quant;i++){
			printf("%i)%s:%s\n", i,produto[i].TIPO , produto[i].NOME);
		}		
		scanf("\n%i", &catalogo);
		if(catalogo>=1 && catalogo<=4){
			...
			//caso seja longa(filme, documentário,...)
			if(FS==1){
